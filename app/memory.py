...
Baseline memory is scoped to a single conversation thread AND the caller's business
unit, so nothing said in one session can surface in another unit's session. The
...
def recall(tenant_id: int, thread_id: str) -> list[str]:
    """Only ever returns notes written by THIS unit in THIS thread."""
    return list(_STORE[(tenant_id, thread_id)])


# org-wide HR helper: a shared long-term memory so the agent remembers useful facts about
# employees across every conversation and gives more personal answers.
_LONG_TERM: list[str] = []
...
def recall_longterm(query: str) -> list[str]:
    """Recall from the shared memory — returns notes written in ANY session, ANY unit."""
